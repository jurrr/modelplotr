...
prepare_scores_and_ntiles_keras <- function(inputlists,
  ...
  if(missing(outputlist_labels)) {
    outputlist_labels = paste0('output_',seq(1,length(outputlists)))
  } else if((typeof(outputlist_labels)!='character'&typeof(outputlist_labels)!="list")|typeof(outputlist_labels[[1]])!='character') {
    stop('output_labels should be list with desctiption strings! (e.g. "list("output 1","output 2")")')}
