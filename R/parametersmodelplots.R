...
customize_plot_text <- function(plot_input=plot_input){
  ...
  plot_text$cumgains$annotationtext <- "When we select &PCTNTL with the highest probability according to &MDL, this selection holds &CUMGAINS of all &YVAL cases in &DS."
  ...
  plot_text$cumlift$annotationtext <- "When we select &PCTNTL with the highest probability according to model &MDL in &DS, this selection for &YVAL cases is &CUMLIFT times better than selecting without a model."
  ...
  plot_text$response$annotationtext <- "When we select ntile &NTL according to model &MDL in dataset &DS the %% of &YVAL cases in the selection is &RESPONSE"
  ...
  plot_text$cumresponse$annotationtext <- "When we select ntiles 1 until &NTL according to model &MDL in dataset &DS the %% of &YVAL cases in the selection is &CUMRESPONSE."
  ...
  plot_text$profit$annotationtext <- "When we select ntiles 1 until &NTL according to model &MDL in dataset &DS the profit is &PROFIT."
  ...
  plot_text$roi$annotationtext <- "When we select ntiles 1 until &NTL according to model &MDL in dataset &DS the return on investment is &ROI."
  ...
  plot_text$costsrevs$annotationtext <- "When we select ntiles 1 until &NTL according to model &MDL in dataset &DS the revenues are &REVENUES."
...
setplotparams <- function(plot_input,plottype,custom_line_colors,plot_text) {
  ...
  pp$cumgains$annolabelfmt <- 'scales::percent_format(accuracy=0.01)'
  ...
  pp$response$annolabelfmt <- 'scales::percent_format(accuracy=0.001)'
  ...
  pp$cumresponse$annolabelfmt <- 'scales::percent_format(accuracy=0.001)'
  ...
  pp$roi$annolabelfmt <- 'scales::percent_format(accuracy=0.001)'
  ...
  pp$costsrevs$linetypes <- c(rep('solid',pp$scope$nlevels),'dashed',rep('dotted',pp$costsrevs$nreflevels))
  pp$costsrevs$alphas <- c(rep(1,pp$scope$nlevels),1,rep(1,pp$roi$nreflevels))
  pp$costsrevs$linecols <- c(pp$scope$levelcols,'gray',pp$costsrevs$reflevelcols)
  pp$costsrevs$linesizes <- c(rep(1,pp$scope$nlevels),1.2,rep(0.7,pp$costsrevs$nreflevels))
...
annotate_plot <- function(plot=plot,highlight_input=plot_input_prepared,
  ...
  if(!is.na(highlight_ntile)) {
    ...
    if (highlight_ntile<1 | (highlight_ntile>pp$scope$ntiles & highlight_ntile!='max')) {
      stop(paste0("Value for highlight_ntile not valid! Choose ntile value to highlight in range [1:",pp$scope$ntiles,"]
                  or use highlight_ntile='max' for maximum value highlighting in profit/roi plots"))
    }
    ...
    if(highlight_how %in% c('plot','plot_text')){
      ...
      if(highlight_ntile == 'max'){
        highlight_input = highlight_input %>% dplyr::group_by(legend) %>%
          dplyr::filter(refline == 0 & plotvalue==max(plotvalue)) %>%
      ...
      if(min(highlight_ntile_num) == max(highlight_ntile_num) & highlight_ntile_num[1] %%  pp$scope$xlabper == 0){
        ...
        plot <- plot  +
          ...
          ggplot2::scale_x_continuous(name=pp$scope$ntiles_label, breaks=xbreaks,labels=xbreaks,expand = c(0, 0.02))
      }else{
        xbreaks <- seq((1-pp$scope$ntile0)*pp$scope$xlabper,pp$scope$ntiles+pp$scope$ntile0,pp$scope$xlabper)
        ...
        plot <- plot  +
          ...
          ggplot2::geom_label(data=highlight_input %>% dplyr::filter(ntile %in% highlight_ntile_num & refline==0),
                              ggplot2::aes(x=highlight_ntile_num,y=-Inf,label = highlight_ntile_num),fill="white",vjust=0.2,fontface = "bold",alpha=0.8,show.legend = FALSE)
      }
    ...
    annovalues <- highlight_input %>%
      ...
      dplyr::mutate(xmin=rep(0,pp$scope$nlevels),
                    ...
                    DS=dataset_label,
                    YVAL=target_class,
                    CUMGAINS=scales::percent_format(accuracy = 0.01)(plotvalue),
                    CUMLIFT=scales::comma_format(accuracy = 0.1)(plotvalue),
                    RESPONSE=scales::percent_format(accuracy = 0.001)(plotvalue),
                    CUMRESPONSE=scales::percent_format(accuracy = 0.001)(plotvalue),
                    PROFIT=scales::dollar_format(prefix = "€")(plotvalue),
                    ROI=scales::percent_format(accuracy = 0.01)(plotvalue),
                    REVENUES=scales::dollar_format(prefix = "€")(plotvalue),
                    # replace the placeholders for values in the annotation text per plot type
                    annotationtext = eval(parse(text=paste0("sprintf('",stringr::str_replace_all(pp$scope$annotationtext,'&[A-Z]+','%s'), " ', ",
                                                            paste(substr(unlist(stringr:: str_extract_all(pp$scope$annotationtext,'&[A-Z]+')),2,100),collapse = ', '),')'))))
